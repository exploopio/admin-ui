...
The Admin UI provides operational management for the Rediver platform's shared agent infrastructure:
...
3. API key is stored in memory (not persisted for security)
...
**Note:** API key is stored in memory only, not in localStorage or cookies (for security).

## Security

- API keys use bcrypt hashing (SEC-C01)
- Account lockout after 10 failed attempts (SEC-H01)
- All admin actions are logged for audit
- Session is memory-only (no persistent storage)
...
- [Platform Agents Overview](../docs/features/platform-agents.md)
- [Platform Admin Guide](../docs/guides/platform-admin.md)
- [Admin CLI Documentation](../docs/guides/platform-admin.md#cli-reference)
- [Full Admin UI Documentation](../docs/admin-ui/index.md)
